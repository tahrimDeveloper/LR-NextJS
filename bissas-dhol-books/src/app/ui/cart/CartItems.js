...
const CartItems = () => {
  ...
  return (
    <div className="w-full max-w-xs bg-gray-400 text-black shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <h2 className="text-2xl pb-4">{cartData.length} Items in Cart</h2>
      <ul>
        {cartData.map(item => (
          <li key={item.id} className="p-2 m-2 rounded-md flex bg-gray-200">
            <div className="bg-black text-white text-center h-[30px] w-[30px] rounded-full pt-1">
              {item.type === 'Buy' ? 'B' : 'R'}
            </div>
